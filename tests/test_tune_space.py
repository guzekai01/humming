...
def test_bk64_wk64_8bit_broken_family_boundaries():
    fp8_meta = _w2_meta()
    ...
    assert reason == "bk64-wk64-8bit-wrong-result"
    assert (
...
def test_golden_counts():
    ...
    assert len(dense_candidates) == 2632
    assert len(masked_candidates) == 598
