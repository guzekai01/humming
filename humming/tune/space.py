...
def _moe_warp_spec_broken(meta: "HummingLayerMeta", gemm_type: GemmType, config: dict) -> bool:
...
def _bn64_small_bm_broken(meta: "HummingLayerMeta", gemm_type: GemmType, config: dict) -> bool:
    block_m, block_n, _ = config["block_shape"]
    return block_n == 64 and block_m <= 32


def _wn64_8bit_broken(meta: "HummingLayerMeta", gemm_type: GemmType, config: dict) -> bool:
...
def _stream_k_values(meta: "HummingLayerMeta", gemm_type: GemmType) -> list[bool]:
    values = [False]
...
class DeviceSearchSpace:
    ...
    @classmethod
    def is_valid(cls, meta: "HummingLayerMeta", gemm_type: GemmType, config: dict) -> bool:
        block_m, block_n, block_k = config["block_shape"]
